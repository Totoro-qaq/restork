...
function replayBoot(root: HTMLElement, reduceMotion: boolean): void {
...
function animateShell(root: HTMLElement, reduceMotion: boolean): gsap.MatchMedia | null {
...
function installParticleNetwork(
  ...
  const resize = (): void => {
    ...
    const count = Math.min(48, Math.max(20, Math.floor((width * height) / 45_000)));
    ...
    const columnCount = Math.min(9, Math.max(5, Math.round(width / 220)));
    columns = Array.from({ length: columnCount }, () => ({
      x: Math.round(Math.random() * width),
      y: Math.random() * height * 1.2 - height * 0.15,
      speed: 34 + Math.random() * 54,
      length: 9 + Math.round(Math.random() * 12),
      step: 15,
    }));
  };
  const draw = (timestamp: number): void => {
    ...
    for (const column of columns) {
      ...
      for (let index = 0; index < column.length; index += 1) {
        const y = column.y - index * column.step;
        if (y < -20 || y > height) continue;
        const fade = 1 - index / column.length;
        context.globalAlpha = 0.05 + fade * 0.18;
        context.fillStyle = index === 0 ? secondary : primary;
        const glyphIndex = Math.abs(Math.floor(column.x + index * 7 + column.y / 26)) % glyphs.length;
        context.fillText(glyphs.charAt(glyphIndex), column.x, y);
      }
    }
    for (let left = 0; left < motes.length; left += 1) {
      const a = motes[left];
      for (let right = left + 1; right < motes.length; right += 1) {
        const b = motes[right];
        const dx = a.x - b.x;
        const dy = a.y - b.y;
        const distanceSquared = dx * dx + dy * dy;
        if (distanceSquared > 19_044) continue;
        const distance = Math.sqrt(distanceSquared);
        context.globalAlpha = (1 - distance / 138) * 0.18;
    ...
    for (const mote of motes) {
      ...
      if (mote.shape === 0) {
        context.globalAlpha = 0.34 + near * 0.5;
        ...
        context.globalAlpha = 0.24 + near * 0.3;
...
export function configureCyberpunkTheme(root: HTMLElement, theme: string | undefined): () => void {
  ...
  if (channelPicker) {
    bind(channelPicker, "change", () => {
      ...
      if (shell && !window.matchMedia("(prefers-reduced-motion: reduce)").matches) {
        gsap.fromTo(shell, { x: -3 }, { x: 0, duration: 0.34, ease: "elastic.out(1, 0.3)", clearProps: "transform" });
      }
    });
  }
  if (fxPicker) {
  ...
  const media = animateShell(root, reduceMotion);
  const canvas = root.querySelector<HTMLCanvasElement>(".cyber-net");
  const stopNetwork = canvas
    ? installParticleNetwork(canvas, () => document.documentElement.dataset.cyberFx === "full" && !reduceMotion)
    : () => undefined;
  if (!reduceMotion) {
  ...
  if (replay) bind(replay, "click", () => { replayBoot(root, reduceMotion); });
  const boot = root.querySelector<HTMLElement>(".cyber-boot");
  const skipBoot = (): void => {
    if (!boot) return;
    gsap.killTweensOf(boot);
    gsap.set(boot, { autoAlpha: 0 });
    boot.hidden = true;
  };
  ...
  return () => {
    ...
    stopNetwork();
    media?.revert();
    gsap.killTweensOf(root.querySelectorAll(".cyber-fx, .cyber-boot, .nav-item, .paper-card"));
