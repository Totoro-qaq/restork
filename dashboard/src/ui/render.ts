...
import { canRetryRun, isRunActive } from "../runState";
...
function providerFailureLabel(kind: string, locale: Locale): string | null {
  const labels: Record<string, [string, string]> = {
...
function stopReasonLabel(reason: string | null, locale: Locale, events: RunEvent[] = []): string {
  ...
  return tr(locale, "This task is no longer running.", "这项任务已不再运行。");
}

function runOutcomeMarkup(run: RunListEntry, events: RunEvent[], locale: Locale): string {
