...
cargo run --manifest-path rust/Cargo.toml --bin restorkd -- \
  serve --port 0 --vault-dir /path/to/vault
```
...
The default quick-capture inbox is `Restork Tasks.md` within the configured Vault and is created only
after its exact preview is approved. Radar `make task` produces the same preview and never writes
immediately.
