...
- 对话回合结束后，若本地匹配命中某已启用技能，在回合下方显示一条建议行（同 chip 语法）：「下一轮用 cobsidian 整理？」。
- 点确认 = 下一次提案/Run 携带该技能；不确认则无任何效果。模型输出不能直接激活技能。
...
1. keywords 缺失时是否用 description 的 TF 高频词兜底，还是干脆不匹配？
2. 同名技能再次导入：默认新修订还是强制改名？（倾向新修订 + 预览 diff）
3. ⌘K 条目是否允许技能声明多个动作（如 ppt-master 的「新建」与「续写」）？首版只允许一个。
