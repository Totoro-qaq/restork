...
- 状态：Gate 1–2 已实现；Gate 3–4 代码已落地，正式发布仍等待签名凭据与三平台干净机器验收。开始页首屏于 2026-08-13 由 [start-page-restraint.zh-CN.md](./start-page-restraint.zh-CN.md) 修正 A2 / A5 / A6
- 日期：2026-08-11；开始页约束修正 2026-08-13
- 适用版本：0.2.x 规划
...
（知识库未连接、有进行中或待审批时才出现状态行；不含模型名）
```

问候、模式控件和状态行的现行合同见 [start-page-restraint.zh-CN.md](./start-page-restraint.zh-CN.md)。下文 A2 / A5 以该修正为准。
...
- Provider/Profile 使用已配置记录的下拉选择，不要求用户手填模型 ID。未配置时不伪造 DeepSeek 选项。
- 模型只出现在输入区的下拉框，不出现在状态行。
...
状态行是例外列表，不是固定四项。不显示当前模型（模型只在下拉框）。仅在下列情况渲染对应项：

- 知识库未连接
- 进行中的 Run 数量大于 0
- 待确认数量大于 0（可附最近到期时间）

零项则整行不出现。「0 个进行中」不是信息。不显示内部枚举、数据库字段或技术缩写。待确认数量大于 0 时使用提醒色，但不持续闪烁。
...
- 成功完成一次 Run 后折叠为 `<details>`，不从开始页删除；帮助页仍可找到示例。
...
- **START-008**：状态行只反映真实例外（未连接知识库、进行中 > 0、待确认 > 0），数据来自 Core；不含模型名。完整合同见开始页克制 Spec。
